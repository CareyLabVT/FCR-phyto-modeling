...
movement <- function(inds, yloc = 2, ymax = 9.3){
    ...
    dens         <- (999.83952 + 16.945176*env[,1] - 0.0079870401*env[,1]^2 - 0.000046170461*env[,1]^3 + 0.00000010556302*env[,1]^4 -
0.00000000028054253*env[,1]^5)/(1 + 0.016879850*env[,1]) #Kell equation for water density (Kell, 1975)
    log_ratio_visc         <- exp((20 - env[,1])/(env[,1] + 96)*(1.2364 - 0.00137*(20 - env[,1]) + 0.0000057*(20 - env[,1])^2)) #stuck here figuring out how to get back to viscosity at target temp from log10(ratio of viscosity at target temp/viscosity at 20 degrees C)
    visc         <- exp(log_ratio_visc)
...
for(i in 1:length(depths)){
  ...
  temp.df <- array(data = 0, dim = c(round(temp$TotalConcNoMixed_ugL,0),2))
  temp.df[,1] <- 1 #placeholder for taxon ID or some other trait
  temp.df[,2] <- depths[i]
  
  inds <- rbind(inds, temp.df)
}
colnames(inds) <- c("PFG","yloc")
