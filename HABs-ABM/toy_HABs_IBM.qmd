...
2.  Movement of individuals given static water temp profile using turbulence/diffusion equation
    ...
    7.  Introduce environment that varies over time
        ...
        1.  use GLM-AED output to update water temperature, density, and light (?) once per hour
            ...
            7.  revise model code to pull in GLM-AED output to update water temperature profiles
...
# Death function
# =============================================================================
death <- function(inds, dcol = 6, yloc = 2, ymax = 9.3, pft = 1, graze_d = 0.32, graze_c = 0.27, graze_g = 0.27, light_exp = 8){
  total_inds <- dim(inds)[1] # Get the number of individuals in inds
...
# Update environment
# ========================================================
update_env <- function(env, wtemp, swr, time_steps, ts){
...
while(ts < time_steps){
    ...
    inds            <- death(inds);
    ts              <- ts + 1; 
    env             <- update_env(env, wtemp, swr, time_steps, ts);
    inds_hist[[ts]] <- inds;
    print(ts)
    print(length(inds[,1]))
}
