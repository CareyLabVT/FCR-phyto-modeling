...
2.  Movement of individuals given static water temp profile using turbulence/diffusion equation
    ...
    4.  Edit "movement" function to use eq. 9 instead of random movement
        ...
        2.  next, worry about random walk to simulate turbulent transport --\> I need velocity to do this I think; bring up in plumbing mtg

    5.  Edit "death" function

        1.  agents that reach the sediments die - check

        2.  agents can also senesce and die; this requires a column to keep track of cell age, and different taxa could have different age limits; check Reynolds et al. 2006 for estimates for appropriate agent lifespans
...
# Death function
# =============================================================================
death <- function(inds, dcol = 6, yloc = 2, ymax = 9.3){
  total_inds <- dim(inds)[1] # Get the number of individuals in inds
...
for(i in 1:length(depths)){
  ...
  temp.df.d <- array(data = 0, dim = c(round(temp$BrownAlgae_ugL,0),6))
  ...
  temp.df.c <- array(data = 0, dim = c(round(temp$Bluegreens_ugL,0),6))
  ...
  temp.df.g <- array(data = 0, dim = c(round(temp$GreenAlgae_ugL,0),6))
...
colnames(inds) <- c("PFG","yloc","cell_diam","cell_dens","cell_shape","dcol")
