...
wq_tl <- wq_kludge %>%
  filter(time >= "2018-01-01" & time <= "2022-12-31")


lakes <- unique(wq_cleaned$site_id)
lakes_with_winter <- c("BARC","SUGG")

#create daily date vector
dates <- get_model_dates(model_start = "2019-01-01", model_stop = "2022-12-31", time_step = 'days')
...
for(i in 1:length(lakes)){
  ...
  med_chla <- na.approx(temp$chla_interp)
...
temp_tl <- temp_cleaned %>%
  filter(time >= "2019-01-01" & time <= "2022-12-31")

lakes <- unique(temp_cleaned$site_id)
lakes_with_winter <- c("BARC","SUGG")
...
for(i in 1:length(lakes)){
  ...
  for(j in 1:length(years)){
    ...
    med_wtemp <- na.approx(temp$wtemp_interp)
...
#linear interpolation to fill in missing values
sec_tl <- sec2 %>%
  filter(DateTime >= "2019-01-01" & DateTime <= "2022-12-31") %>%
  filter(!(Lake == "TOOK" & DateTime == "2019-06-21" & secchiMeanDepth == 3.7)) %>%
  distinct()

lakes <- unique(sec2$Lake)
lakes_with_winter <- c("BARC","SUGG")
...
for(i in 1:length(lakes)){
  ...
  for(j in 1:length(years)){
    ...
    med_secchiMeanDepth <- na.approx(temp$secchiMeanDepth_interp)
    ...
    if(i == 1 & j == 1){
      ...
      if(all(is.na(temp$secchiMeanDepth))) next
...
join_df <- tibble(DateTime = rep(daily_dates$DateTime, times = 7),
                  Lake = rep(lakes, each = nrow(daily_dates)))
...
nuts <- loadByProduct(dpID="DP1.20093.001", site=c("BARC","SUGG","CRAM","LIRO","PRLA","PRPO","TOOK"),
                     startdate="2019-01", enddate="2022-12", 
                     package="expanded", 
                     token = NEON_TOKEN,
                     check.size = F,
                     release = "current",
...
for(i in 1:length(lakes)){
  ...
  for(j in 1:length(years)){
    ...
    mylake <- nuts_tl %>%
      filter(Lake == lakes[i] & year(DateTime) == years[j] & (!is.na(DIN_ugL) & !is.na(SRP_ugL))) 
    ...
    med_DIN_ugL <- na.approx(temp$DIN_ugL_interp)
    med_SRP_ugL <- na.approx(temp$SRP_ugL_interp)
    ...
    if(i == 1 & j == 1){
      ...
      if(all(is.na(temp$DIN_ugL)) | all(is.na(temp$SRP_ugL))) next
...
for(i in 1:length(lakes)){
  ...
  for(j in 1:length(years)){
    ...
    temp <- left_join(subset(daily_dates, year(daily_dates$DateTime) == years[j]), mylake, by = "DateTime") %>%
      mutate(Lake = ifelse(is.na(Lake),lakes[i],Lake)) %>%
      mutate(at_interp = replace(at, cumall(is.na(at)), at[!is.na(at)][1]))
    
    med_at <- na.approx(temp$at_interp)
    ...
    if(i == 1 & j == 1){
      ...
      temp <- left_join(subset(daily_dates, year(daily_dates$DateTime) == years[j] & daily_dates$DateTime %in% mylake$DateTime), mylake, by = "DateTime") %>%
        mutate(Lake = ifelse(is.na(Lake),lakes[i],Lake)) %>%
        mutate(at_interp = replace(at, cumall(is.na(at)), at[!is.na(at)][1]))
...
at_final <- left_join(join_df, at5, by = c("DateTime","Lake")) %>%
...
DataNEONLakes <- read_csv("./Eco-KGML-transfer-learning/data/data_processed/DataNEONLakesWinter.csv") %>%
  arrange(Lake,DateTime) %>%
  mutate(AirTemp_C = at_final$AirTemp_C,
         Flag_AirTemp_C = ifelse(at_final$Interp_Flag_at == TRUE, 1, 0))

ggplot(data = DataNEONLakes, aes(x = DateTime, y = AirTemp_C, group = Flag_AirTemp_C, color = Flag_AirTemp_C))+
  geom_point()+
  facet_wrap(vars(Lake), scales = "free", nrow = 2)+
  theme_bw()
...
for(i in 1:length(lakes)){
  ...
  for(j in 1:length(years)){
    ...
    temp <- left_join(subset(daily_dates, year(daily_dates$DateTime) == years[j]), mylake, by = "DateTime") %>%
      mutate(Lake = ifelse(is.na(Lake),lakes[i],Lake)) %>%
      mutate(sw_interp = replace(sw, cumall(is.na(sw)), sw[!is.na(sw)][1]))
    
    med_sw <- na.approx(temp$sw_interp)
...
sw_final <- left_join(join_df, sw3, by = c("DateTime","Lake")) %>%
  mutate(Flag_Shortwave_Wm2 = ifelse(Interp_Flag_sw == TRUE, 1, 0)) %>%
  arrange(Lake,DateTime)

DataNEONLakes <- read_csv("./Eco-KGML-transfer-learning/data/data_processed/DataNEONLakesWinter.csv") %>%
  arrange(Lake,DateTime) %>%
  mutate(Shortwave_Wm2 = sw_final$Shortwave_Wm2,
         Flag_Shortwave_Wm2 = ifelse(sw_final$Interp_Flag_sw == TRUE, 1, 0))

ggplot(data = DataNEONLakes, aes(x = DateTime, y = Shortwave_Wm2, group = Flag_Shortwave_Wm2, color = Flag_Shortwave_Wm2))+
  geom_point()+
  facet_wrap(vars(Lake), scales = "free", nrow = 2)+
  theme_bw()
